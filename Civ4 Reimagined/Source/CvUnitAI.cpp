...
int CvUnitAI::AI_groupFirstVal()
{
	...
	case UNITAI_SETTLE:
		return 22; // Civ4 Reimagined
		break;

	case UNITAI_WORKER:
		return 21; // Civ4 Reimagined
		break;

	case UNITAI_ATTACK:
		if (collateralDamage() > 0)
		{
			return 16; // Civ4 Reimagined, was 15
		}
		// Civ4 Reimagined
		else if (getUnitClassType() == (UnitClassTypes)GC.getInfoTypeForString("UNITCLASS_MARINE"))
		{
			return 15;
		}
	...
	case UNITAI_ATTACK_CITY:
		if (bombardRate() > 0)
		{
			return 20; //Civ4 Reimagined
		}
		else if (collateralDamage() > 0)
		{
			return 19; //Civ4 Reimagined
		}
		else if (withdrawalProbability() > 0)
		{
			return 18; //Civ4 Reimagined
		}
		else
		{
			return 17; //Civ4 Reimagined
		}
		break;

	case UNITAI_COLLATERAL:
		// Civ4 Reimagined (was 7)
		return 19;
		break;
...
void CvUnitAI::AI_attackMove()
{
	...
	if (plot()->getTeam() == getTeam() && (bDanger || area()->getAreaAIType(getTeam()) != AREAAI_NEUTRAL))
	{
		...
		{
			// Civ4 Reimagined
			if (collateralDamage() == 0 && AI_leaveAttack(2, 55, 105))
				return;
...

	{
		...
		if (bDanger)
		{
			...
			// Civ4 Reimagined
			if (collateralDamage() > 0 && plot()->isCity())
			{
...

	{
		...
		if (bDanger)
		{
			...
			// Civ4 Reimagined
			else if (collateralDamage() == 0 && AI_anyAttack(1, 45))
			{
				return;
			}
		...
		if (iStepDistToTarget <= 2)
		{
			...
			if (bTargetTooStrong)
			{
				...
				else
				{
					...
					if (pBombardCity != NULL && pBombardCity->getBuildingDefense() > 0)
					{
						if (canBombard(plot()))
						{
			...
			if (iStepDistToTarget == 1)
			{
				...
				else if (AI_bombardCity()) // Civ4 Reimagined
					return;
...
// Mostly added getNumDefenders restrictions because AI used to leave single collateral units undefended.
void CvUnitAI::AI_collateralMove()
{
	PROFILE_FUNC();

	// K-Mod!
	if (plot()->isCity() && AI_defensiveCollateral(51, 1))
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_defensiveCollateral",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	}
	// K-Mod end
	...
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_leaveAttack",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	}

	if (AI_guardCity(false, true, 1))
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_guardCity",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	}

	if (AI_heal(30, 1))
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_heal",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	}
	
	// Civ4 Reimagined
	if (plot()->getNumDefenders(getOwner()) > 3 && AI_bombardCity())
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_bombardCity",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	}

	if (plot()->getNumDefenders(getOwner()) > 3 && AI_cityAttack(1, 35))
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_cityAttack",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	}
	...
	if (plot()->getNumDefenders(getOwner()) > 3 && AI_anyAttack(1, 55, 0, 2))
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_anyAttack1",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	}

	if (plot()->getNumDefenders(getOwner()) > 3 && AI_anyAttack(1, 35, 0, 3))
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_anyAttack2",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	}
	*/
	...
	// K-Mod
	{
		...
		if (plot()->getOwnerINLINE() != getOwnerINLINE()) //Civ4 Reimagined: Only use this in enemy lands
		{
			...
			do
			{
				...
				{
					if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_anyAttack",plot()->getX_INLINE(),plot()->getY_INLINE());
					return;
				}
	...
	if (AI_heal())
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_heal2",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	}
	...
	/* //Removed by Civ4 Reimagined
	if (plot()->getNumDefenders(getOwner()) > 2 && AI_cityAttack(2, 50))
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_cityAttack2",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	} */
	...
	if (AI_retreatToCity())
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_retreatToCity",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	}

	// K-Mod
	if (AI_handleStranded())
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_handleStranded",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	// K-Mod end

	if (AI_safety())
	{
		if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_safety",plot()->getX_INLINE(),plot()->getY_INLINE());
		return;
	}

	getGroup()->pushMission(MISSION_SKIP);
	if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_SKIP",plot()->getX_INLINE(),plot()->getY_INLINE());
	return;
}
...
/************************************************************************************************/
	...
	if (collateralDamage == 0 && bDanger)
	{
		...
		if (AI_chokeDefend())
		{
			if (gPlayerLogLevel > 2) logBBAI("Plot %d,%d: AI_chokeDefend",plot()->getX_INLINE(),plot()->getY_INLINE());
			return;
		}
	}
...
/************************************************************************************************/
	...
	// Civ4 Reimagined
	if (collateralDamage() == 0 && AI_leaveAttack(2, 55, 150))
	{
		return;
	}
	
	// Civ4 Reimagined
	if (plot()->isCity() && AI_leaveAttack(1, 55, 150))
	{
		return;
	}
	
	// Civ4 Reimagined
	if (collateralDamage() == 0 && AI_chokeDefend())
	{
		return;
	}
...
/********************************************************************************/
	...
	if (!(isHuman()) && (AI_getUnitAIType() == UNITAI_EXPLORE_SEA))
	{
		...
		if (pWaterArea != NULL)
		{
			if ((kOwner.AI_totalWaterAreaUnitAIs(pWaterArea, UNITAI_EXPLORE_SEA) > kOwner.AI_neededExplorers(pWaterArea)) || (GET_PLAYER(getOwnerINLINE()).getCurrentEra() > 3 && getInvisibleType() == NO_INVISIBLE))
			{
				if (getUnitInfo().getBaseTrade() > 0 && getUnitInfo().getTradeMultiplier() > 0)
				{
					if( gUnitLogLevel > 2 ) logBBAI("UNITAI_EXPLORE_SEA changes to UNITAI_MERCHANT");
					AI_setUnitAIType(UNITAI_MERCHANT);
					return;
				}
			}
		}
...
/********************************************************************************/
	...
	// Civ4 Reimagined
	if (AI_missileLoad(UNITAI_MISSILE_CARRIER_SEA, -1 ,true))
	{
		return;
	}
...
int CvUnitAI::AI_promotionValue(PromotionTypes ePromotion)
{
	...
	if (iTemp != 0)
	{
		...
		{
			// Civ4 Reimagined: changed this to 1 instead of 2/3
			iValue += (iTemp * 1);
		}
		// Civ4 Reimagined
		else if (AI_getUnitAIType() == UNITAI_RESERVE)
		{
			iValue += (iTemp / 3);
		}
		else
...
/************************************************************************************************/
	...
	if (iTemp != 0)
	{
		if (m_pUnitInfo->getUnitAIType(UNITAI_ATTACK) || m_pUnitInfo->getUnitAIType(UNITAI_ATTACK_CITY) || m_pUnitInfo->getUnitAIType(UNITAI_ATTACK_CITY_LEMMING))
		{
			...
			// Civ4 Reimagined
			if ( GC.getGameINLINE().getCurrentEra() < 2 && getExtraCityAttackPercent() >= 20)
			{
				iTemp /= 10;
			}
...
{	
	...
	if (canDiscover(plot()))
	{
		...
		// Removed by Civ4 Reimagined: Overflow goes in next tech --> no waste
...
bool CvUnitAI::AI_defend()
{
	...
	// Civ4 Reimagined infinite loop
	//logBBAI("%d search range: %d from (%d,%d)", getID(), iSearchRange, plot()->getX_INLINE(),plot()->getY_INLINE());
				...
				if (AI_plotValid(pLoopPlot))
				{
					if (AI_defendPlot(pLoopPlot))
					{
						if (!(pLoopPlot->isVisibleEnemyUnit(this)))
						{
							...
							// Civ4 Reimagined: Added canMoveThrough (might be a reason for OOS errors!!)
							if (!atPlot(pLoopPlot) && getGroup()->canMoveThrough(pLoopPlot) && generatePath(pLoopPlot, 0, true, &iPathTurns, 1))
							{
								if (iPathTurns <= 1)
								{
...
/************************************************************************************************/
		...
		// Civ4 Reimagined infinite loop
		// Todo: implement counter. Increment it whenever this unit passes this part of the code. If counter big enough, tell the unit to skip turn. Reset counter every turn.
		//logBBAI("%d wants to move to (%d,%d)", getID(), pBestPlot->getX_INLINE(),pBestPlot->getY_INLINE());
		return true;
	}
...
bool CvUnitAI::AI_bombardCity()
{
	...
	// Civ4 Reimagined: This calculation doesn't work anymore with our new combat system.
	/*
	...
	*/
	...
	// Civ4 Reimagined: Only skip when bombarding won't help us.
	if (pBombardCity->getBuildingDefense() == 0)
	{
		if( gUnitLogLevel > 2 ) logBBAI("      Stack skipping bombard of %S because city has no defenses anyway", pBombardCity->getName().GetCString());
		return false;
	}
...
bool CvUnitAI::AI_anyAttack(int iRange, int iOddsThreshold, int iFlags, int iMinStack, bool bAllowCities, bool bFollow)
{
	...
	// Civ4 Reimagined
	if (collateralDamage() > 0 && (iRange > 1 || !plot()->isCity()))
		return false;
...
bool CvUnitAI::AI_leaveAttack(int iRange, int iOddsThreshold, int iStrengthThreshold)
{
	...
	// Civ4 Reimagined
	if (collateralDamage() > 0 && (iRange > 1 || pCity == NULL))
		return false;
	...
	for (int iDX = -(iMaxRange); iDX <= iMaxRange; iDX++)
	{
		for (int iDY = -(iMaxRange); iDY <= iMaxRange; iDY++)
		{
			...
			if (pLoopPlot != NULL && AI_plotValid(pLoopPlot))
			{
				...
				if (pBombardCity != NULL && isEnemy(pBombardCity->getTeam(), pLoopPlot) && pBombardCity->getNumBuildings() > 0) // Civ4 Reimagined
				{
					int iPathTurns;
	...
	if ((pBestPlot != NULL) && (pBestBombardPlot != NULL))
	{
		if (atPlot(pBestBombardPlot))
		{
			...
			if (bBombardUnitCanBombardNow && pGroup->canBombard(pBestBombardPlot))
			{
				...
				if (pBombardCity == NULL) // Civ4 Reimagined
				{
				...
				{
					if (pBombardCity->getBuildingDefense() == 0 && GET_PLAYER(getOwnerINLINE()).AI_plotTargetMissionAIs(pBombardCity->plot(), MISSIONAI_BLOCKADE, getGroup(), 3) == 0)
					{
...
bool CvUnitAI::AI_improveCity(CvCity* pCity)
{
	...
	if (AI_bestCityBuild(pCity, &pBestPlot, &eBestBuild, NULL, this))
	{
		...
		/* Civ4 Reimagined To-Do: Let's try to skip this part. AI values roads too high
...
bool CvUnitAI::AI_irrigateTerritory()
{
	...
	if (pBestPlot != NULL)
	{
		...
		//getGroup()->pushMission(MISSION_ROUTE_TO, pBestPlot->getX_INLINE(), pBestPlot->getY_INLINE(), 0, false, false, MISSIONAI_BUILD, pBestPlot);
		getGroup()->pushMission(MISSION_MOVE_TO, pBestPlot->getX_INLINE(), pBestPlot->getY_INLINE(), 0, false, false, MISSIONAI_BUILD, pBestPlot); // Civ4 Reimagined
			...
			if (pLoopPlot != NULL)
			{
				...
				if (canMoveInto(pLoopPlot, true))
				{
					...
					//if (pLoopPlot->isWater() || (iPotentialAttackers > 0) || pLoopPlot->isAdjacentTeam(getTeam()))
					{
						...
						// Civ4 Reimagined: Fighters prefer attacking sea units
						if (pLoopPlot->isWater() && canAirDefend())
						{
							iStrikeValue *= 3;
						}
				...
				if (canAirBombAt(plot(), pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE()))
				{
					if (pLoopPlot->isCity(false))
					{
						...
						iBombValue = pCity->getPopulation();
						iBombValue += pCity->getNumBuildingsAtDoTurn() * 3;
						iBombValue += pCity->getBuildingDefense() * (iAssaultEnRoute > 0 ? 3 : 1);
					}
					else
					{
...
bool CvUnitAI::AI_nuke()
{
	...
	for (PlayerTypes i = (PlayerTypes)0; i < MAX_CIV_PLAYERS; i = (PlayerTypes)(i+1))
	{
		...
		{
			// Civ4 Reimagined: Don't use nukes lightly
			if (kOwner.AI_getMemoryCount(i, MEMORY_NUKED_US) > 0 || (kOwner.getNumNukeUnits() > std::max(3, kOwner.getNumCities() / 3) && (iWarRating < -25 || kOwner.AI_isDoVictoryStrategy(AI_VICTORY_CONQUEST4) || kOwner.AI_isDoVictoryStrategy(AI_VICTORY_DOMINATION4))))
			{
	...
	if (pBestTarget)
	{
		...
		if (gUnitLogLevel > 2)
			logBBAI("%S launches an ICBM (War Success Rating: %d)", GET_PLAYER(getOwnerINLINE()).getCivilizationDescription(0), GET_TEAM(GET_PLAYER(getOwnerINLINE()).getTeam()).AI_getWarSuccessRating());
...
bool CvUnitAI::AI_nukeRange(int iRange)
{
	...
	if (iNukeValue > iThresholdValue)
	{
		...
		if (gUnitLogLevel > 2)
			logBBAI("%S launches a Tactical Nuke (War Success Rating: %d)", GET_PLAYER(getOwnerINLINE()).getCivilizationDescription(0), GET_TEAM(GET_PLAYER(getOwnerINLINE()).getTeam()).AI_getWarSuccessRating());
			...
			for (int jX = -nukeRange(); bValid && jX <= nukeRange(); jX++)
			{
				for (int jY = -nukeRange(); bValid && jY <= nukeRange(); jY++)
				{
					...
					if (bValid && pLoopPlot->isOwned())
					{
						...
						// Civ4 Reimagined: Don't use nukes lightly (check only for tactical nukes, ICBM check in AI_nuke() )
						if (airRange() > 0 && (kOwner.AI_getMemoryCount(pLoopPlot->getOwnerINLINE(), MEMORY_NUKED_US) <= 0) && (GET_TEAM(kOwner.getTeam()).AI_getWarSuccessRating() >= -25) && !kOwner.AI_isDoVictoryStrategy(AI_VICTORY_CONQUEST4) && !kOwner.AI_isDoVictoryStrategy(AI_VICTORY_DOMINATION4))
							bValid = false;
					...
					// Civ4 Reimagined: Don't use nukes lightly (check only for tactical nukes, ICBM check in AI_nuke() )
					if (bValid && airRange() > 0 && (GET_TEAM(kOwner.getTeam()).AI_getWarSuccessRating() >= -25) && !kOwner.AI_isDoVictoryStrategy(AI_VICTORY_CONQUEST4) && !kOwner.AI_isDoVictoryStrategy(AI_VICTORY_DOMINATION4))
						bValid = false;
...
int CvUnitAI::AI_calculatePlotWorkersNeeded(CvPlot* pPlot, BuildTypes eBuild)
{
	...
	int iWorkRate = workRate(true, eBuild);
