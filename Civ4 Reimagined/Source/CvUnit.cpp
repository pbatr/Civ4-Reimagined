...
void CvUnit::init(int iID, UnitTypes eUnit, UnitAITypes eUnitAI, PlayerTypes eOwner, int iX, int iY, DirectionTypes eFacingDirection)
{
	...
	// Civ4 Reimagined
	if (m_pUnitInfo->isNoUpkeep())
	{
		GET_PLAYER(getOwnerINLINE()).changeBaseFreeUnits(1);
	}
...
/********************************************************************************/
	...
	else if (iOurDamage > 0)
	{
		if (iTheirRoundDamage > 0)
		{
			pInterceptor->changeExperience(GC.getDefineINT("EXPERIENCE_FROM_WITHDRAWL"), maxXPValue(pInterceptor->getOwnerINLINE()), true, pPlot->getOwnerINLINE() == pInterceptor->getOwnerINLINE(), !isBarbarian());
		}
	}
	else if (iTheirDamage > 0)
	{
		changeExperience(GC.getDefineINT("EXPERIENCE_FROM_WITHDRAWL"), pInterceptor->maxXPValue(getOwnerINLINE()), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pInterceptor->isBarbarian());
	}
		...
		if (GC.getGameINLINE().getSorenRandNum(GC.getCOMBAT_DIE_SIDES(), "Combat") < iDefenderOdds)
		{
			if (getCombatFirstStrikes() == 0)
			{
				if (getDamage() + iAttackerDamage >= maxHitPoints() && GC.getGameINLINE().getSorenRandNum(100, "Withdrawal") < withdrawalProbability())
				{
					...
					changeExperience(GC.getDefineINT("EXPERIENCE_FROM_WITHDRAWL"), pDefender->maxXPValue(getOwnerINLINE()), true, pPlot->getOwnerINLINE() == getOwnerINLINE(), !pDefender->isBarbarian());
...
void CvUnit::updateCombat(bool bQuick)
{
	...
	if (bFinish)
	{
		...
		if (isDead())
		{
			...
			if (isAnimal())
			{
				if (! GET_TEAM(pDefender->getTeam()).isTechBoosted((TechTypes)GC.getInfoTypeForString("TECH_ANIMAL_HUSBANDRY")))
				{
					GET_TEAM(pDefender->getTeam()).setTechBoosted((TechTypes)GC.getInfoTypeForString("TECH_ANIMAL_HUSBANDRY"), pDefender->getOwnerINLINE(), true);
				}
			}
			...
			// Civ4 Reimagined
			gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GET_PLAYER(getOwnerINLINE()).getCurrentEra()).getAudioUnitDefeatScript(), MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
			...
			// Civ4 Reimagined
			gDLL->getInterfaceIFace()->addHumanMessage(pDefender->getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GET_PLAYER(pDefender->getOwnerINLINE()).getCurrentEra()).getAudioUnitVictoryScript(), MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
		...
		else if (pDefender->isDead())
		{
			...
			{
				if (! GET_TEAM(getTeam()).isTechBoosted((TechTypes)GC.getInfoTypeForString("TECH_ANIMAL_HUSBANDRY")))
				{
					GET_TEAM(getTeam()).setTechBoosted((TechTypes)GC.getInfoTypeForString("TECH_ANIMAL_HUSBANDRY"), getOwnerINLINE(), true);
				}
			}
			...
			// Civ4 Reimagined
			gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GET_PLAYER(getOwnerINLINE()).getCurrentEra()).getAudioUnitVictoryScript(), MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
			...
			// Civ4 Reimagined
			gDLL->getInterfaceIFace()->addHumanMessage(pDefender->getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GET_PLAYER(pDefender->getOwnerINLINE()).getCurrentEra()).getAudioUnitDefeatScript(), MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
		...
		else
		{
			...
			// Civ4 Reimagined
			if (GC.getUnitInfo(getUnitType()).getFirstStrikes() > 0)
			{
				gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_COLLATERAL", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
			}
			else
			{
				gDLL->getInterfaceIFace()->addHumanMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_OUR_WITHDRAWL", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
			...
			gDLL->getInterfaceIFace()->addHumanMessage(pDefender->getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_THEIR_WITHDRAWL", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_YELLOW"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
...
bool CvUnit::canEnterTerritory(TeamTypes eTeam, bool bIgnoreRightOfPassage) const
{
	...
	// Civ4 Reimagined
	if (getDomainType() == DOMAIN_SEA && GET_PLAYER(getOwnerINLINE()).isExploreRivalSea())
	{
		return true;
	}
	...
	if (bCanEnterArea)
	{
		if (pPlot->getFeatureType() != NO_FEATURE)
		{
			if (m_pUnitInfo->getFeatureImpassable(pPlot->getFeatureType()))
			{
				...
				if ((NO_TECH == eTech || !GET_TEAM(getTeam()).isHasTech(eTech)) && !(pPlot->isRoute())) //Civ4 Reimagined: Units can enter impassable features with roads (Chariots)
				{
...
void CvUnit::scrap()
{
	...
	if (gUnitLogLevel > 2)
		logBBAI("    %S disbands %S", GET_PLAYER(getOwnerINLINE()).getCivilizationDescription(0), getName(0).GetCString());
...
bool CvUnit::canNuke(const CvPlot* pPlot) const
{
	...
	BuildingTypes eBuilding = (BuildingTypes)GC.getInfoTypeForString("BUILDING_MISSILE_SILO");

	if (nukeRange() == -1)
	{
		return false;
	}
	// ICBMs need Missile Silos
	if (airRange() == 0 && ((pPlot->getPlotCity() == NULL) || (pPlot->getPlotCity()->getNumRealBuilding(eBuilding) < 1)))
	{
		return false;
	}
	// Civ4 Reimagined END
...
bool CvUnit::paradrop(int iX, int iY)
{
	...
	// Civ4 Reimagined: Paratroopers can now attack in the same turn after a drop
	//setMadeAttack(true);
...
bool CvUnit::airBomb(int iX, int iY)
{
	...
	{
		// Civ4 Reimagined
		bombardCity(pCity, airBombCurrRate());
	}
	else
	{
		if (pPlot->getImprovementType() != NO_IMPROVEMENT)
...
CvCity* CvUnit::bombardTarget(const CvPlot* pPlot) const
{
	...
	int iBestValue = 0; // Civ4 Reimagined
		...
		if (pLoopPlot != NULL)
		{
			...
			if (pLoopCity != NULL)
			{
				if (pLoopCity->isBombardable(this))
				{
					...
					int iValue = pLoopCity->getNumBuildings(); // Civ4 Reimagined
...
bool CvUnit::canBombard(const CvPlot* pPlot) const
{
	...
	// Civ4 Reimagined: Don't bombard city buildings which we want to conquer anyway.
	// This should actually be in CvUnitAI but I think it will work out better here.
	if (!isHuman())
	{
...
// Changed by Civ4 Reimagined: Bombard() now means destroying buildings instead of city defenses.
bool CvUnit::bombard()
{
	CvPlot* pPlot = plot();
	if (!canBombard(pPlot))
	...
	// Civ4 Reimagined
	bombardCity(pBombardCity, bombardRate());
	...
	*/
	...
	setMadeAttack(true);
	//changeMoves(GC.getMOVE_DENOMINATOR());
	// Civ4 Reimagined: Bombarding costs all movement points
	finishMoves();
...
/************************************************************************************************/
	...
	// Civ4 Reimagined: Add check for coastal raid
	if (!(pPlot->isValidDomainForAction(*this)) && !(canCoastalRaid() && pPlot->isCoastalLand()))
	{
		return false;
	}
...
bool CvUnit::spread(ReligionTypes eReligion)
{
	...
	if (pCity != NULL)
	{
		...
		// Civ4 Reimagined: AverageSpreadResistance
		int iAverageSpreadResistance = 0;
		for (int iI = 0; iI < GC.getNumReligionInfos(); iI++)
		{
			if (pCity->isHasReligion((ReligionTypes)iI) && iI != eReligion)
			{
				iAverageSpreadResistance += pCity->getReligionGrip((ReligionTypes)iI);
			}
		}
		iAverageSpreadResistance /= ((iPresentReligions == 0) ? 1 : iPresentReligions); //Should result in something in the range of 0-200, depending on xml values.
		
		int iSpreadProb = 50 * ((m_pUnitInfo->getReligionSpreads(eReligion) * std::min(40, 20 + pCity->getPopulation())) / 4);
		iSpreadProb *= iMissingReligions;
		iSpreadProb /= GC.getNumReligionInfos();
		iSpreadProb /= (iAverageSpreadResistance + 1);
		
		// Civ4 Reimagined: Later religions have higher chances to spread
		iSpreadProb *= 200 + GC.getReligionInfo(eReligion).getSpreadFactor();
		iSpreadProb /= 200;
		
		// Civ4 Reimagined: Generic modifier for XML tuning.
		iSpreadProb *= GC.getDefineINT("RELIGION_INFLUENCE_UNIT_MODIFIER_PERCENT");
		iSpreadProb /= 100;
		
		// Civ4 Reimagined: Flat % boost for spreading state religion in own empire
		if (GET_PLAYER(getOwnerINLINE()).getStateReligion() == eReligion && pCity->getOwnerINLINE() == getOwnerINLINE())
		{
			iSpreadProb += GC.getDefineINT("RELIGION_INFLUENCE_OWN_STATE_RELIGION_SPREAD");
		}
		
		//int iSpreadProb = iPresentReligions * (m_pUnitInfo->getReligionSpreads(eReligion) + pCity->getPopulation())
		//	+ iMissingReligions * std::min(100, 100 - 10 * iPresentReligions);
		//iSpreadProb /= GC.getNumReligionInfos();
		...
		if (GC.getGameINLINE().getSorenRandNum(100, "Unit Spread Religion") < iSpreadProb)
		{
			...
			// Civ4 Reimagined: Moved chance to remove one of the existing religions (introduced by K-Mod) up for two reasons: Easier probability calculation, and logical consistancy. As of before the religions with a *lower* chance to spread in a city had a *higher* chance to remove other religions.
			...
			for (int iI = 0; iI < GC.getNumReligionInfos(); iI++)
			{
				if (pCity->isHasReligion((ReligionTypes)iI) || iI == eReligion)
				{
					if (pCity != GC.getGame().getHolyCity((ReligionTypes)iI)) // holy city can't lose its religion!
					{
						...
						// Civ4 Reimagined: Generic modifier for XML tuning.
						iInfluence *= (iI == eReligion) ? GC.getDefineINT("REMOVE_RELIGION_UNIT_MODIFIER_PERCENT") : 100;
						iInfluence /= 100;
...
int CvUnit::spreadCorporationCost(CorporationTypes eCorporation, CvCity* pCity) const
{
	...
	// Civ4 Reimagined: Remove spread cost altogether
	return 0;
...
// Civ4 Reimagined
int CvUnit::getGreatPersonBulbScaling() const
{
	double averageEra = GC.getGameINLINE().getCurrentEraAverage() + 0.1; // Old: + 0.25
...
// Civ4 Reimagined: New values with much better scaling into late-game.
int CvUnit::getDiscoverResearch(TechTypes eTech) const
{
	int iResearch;

	iResearch = getGreatPersonBulbScaling(); 

	iResearch *= m_pUnitInfo->getDiscoverMultiplier();
	iResearch /= 100;
...
bool CvUnit::discover()
{
	...
	// Civ4 Reimagined: Overflow from bulbing techs is applied to other techs.
	int discoverResearchPoints = getDiscoverResearch(NO_TECH);
	
	while(discoverResearchPoints > 0)
	{
		eDiscoveryTech = getDiscoveryTech();
		//FAssertMsg(eDiscoveryTech != NO_TECH, "DiscoveryTech is not assigned a valid value");
	
		if (eDiscoveryTech != NO_TECH)
		{
...
int CvUnit::getMaxHurryProduction(CvCity* pCity) const
{
	...
	if (GET_PLAYER(getOwnerINLINE()).getHurryWithGreatPriestsRatio() != 0 && getUnitClassType() == (UnitClassTypes)(GC.getInfoTypeForString("UNITCLASS_PROPHET")))
	{
		UnitTypes eUnittype = (UnitTypes)(GC.getInfoTypeForString("UNIT_ENGINEER"));
		
		int iProductionBonus = GC.getUnitInfo(eUnittype).getBaseHurry() + GC.getUnitInfo(eUnittype).getHurryMultiplier() * pCity->getPopulation();
		iProductionBonus *= GET_PLAYER(getOwnerINLINE()).getHurryWithGreatPriestsRatio();
		iProductionBonus /= 100;
	}
...
{
	CvCity* pOwnCapitalCity;
	CvCity* pCity;
	int iGold;

	pCity = pPlot->getPlotCity();
	pOwnCapitalCity = GET_PLAYER(getOwnerINLINE()).getCapitalCity();
	...
	// Civ4 Reimagined: Moved this up
	iGold = m_pUnitInfo->getBaseTrade();

	if (m_pUnitInfo->isGoldenAge())
	{
		iGold += getGreatPersonBulbScaling(); 
	}

	iGold *= m_pUnitInfo->getTradeMultiplier();
	iGold /= 100;
	
	if (pOwnCapitalCity != NULL)
	{
		// Up to +35% from population
		...
		for (int iI = 0; iI < MAX_PLAYERS; iI++)
		{
			...
			if (kLoopPlayer.isAlive())
			{
				if (kLoopPlayer.getTeam() != getTeam())
				{
					...
					{
						iTempPopulation = kLoopPlayer.getCapitalCity()->getPopulation();
		...
		int iPopulationModifier = 0;
		
		if (iBestPopulation > 0)
		{
			iPopulationModifier = std::min(35, (35 * pCity->getPopulation()) / iBestPopulation);
		}
		
		// Up to +65% from distance
...
int CvUnit::getEspionagePoints(const CvPlot* pPlot) const
{
	...
	iEspionagePoints = getGreatPersonBulbScaling(); 
	...
	else
	{
		...
		if (GET_PLAYER(getOwnerINLINE()).doEspionageMission(eMission, eTargetPlayer, plot(), iData, this))
		{
			...
			if (!testSpyIntercepted(eTargetPlayer, true, GC.getDefineINT("ESPIONAGE_SPY_MISSION_ESCAPE_MOD")))
			{
				...
				// Civ4 Reimagined: Transport Spy to closest city rather than capital
				CvCity* pCity = GC.getMapINLINE().findCity(getX(), getY(), GET_PLAYER(getOwnerINLINE()).getID(), GET_PLAYER(getOwnerINLINE()).getTeam(), false, false);
				
				if (pCity == NULL)
					pCity = GET_PLAYER(getOwnerINLINE()).getCapitalCity();
				...
				if (NULL != pCity)
				{
					...
					// Civ4 Reimagined
					const iGreatSpyPoints = (100 + GC.getEspionageMissionInfo(eMission).getDifficultyMod()) * GC.getDefineINT("GREAT_SPY_POINTS_PER_MISSION") / 100;

					if (iGreatSpyPoints > 0)
					{
						pCity->instantGreatPeopleProgress((UnitClassTypes)GC.getInfoTypeForString("UNITCLASS_GREAT_SPY"), iGreatSpyPoints);
					}
...
bool CvUnit::build(BuildTypes eBuild)
{
	...
	if (bFinished)
	{
		...
		// Civ4 Reimagined
		if (! GET_TEAM(getTeam()).isTechBoosted((TechTypes)GC.getInfoTypeForString("TECH_BRONZE_WORKING")))
		{
			if (eBuild == (BuildTypes)GC.getInfoTypeForString("BUILD_MINE") && plot()->getBonusType() == (BonusTypes)GC.getInfoTypeForString("BONUS_COPPER"))
			{
				GET_TEAM(getTeam()).setTechBoosted((TechTypes)GC.getInfoTypeForString("TECH_BRONZE_WORKING"), getOwnerINLINE(), true);
			}
		}
	}
...
int CvUnit::upgradePrice(UnitTypes eUnit) const
{
	...
	// Civ4 Reimagined
	iPrice += (iPrice * GET_PLAYER(getOwnerINLINE()).getUnitUpgradeCostModifier() ) / 100;
...
{
	// Civ4 Reimagined: No circumnavigation bonus for Privateers
	if (m_pUnitInfo->isHiddenNationality())
	{
...
int CvUnit::workRate(bool bMax, BuildTypes eBuild) const
{
	...
	int iRateModifier = std::max(0, (GET_PLAYER(getOwnerINLINE()).getWorkerSpeedModifier() + 100));
...
/************************************************************************************************/
			...
			iExtraModifier = pPlot->defenseModifier(getTeam(), (pAttacker != NULL) ? pAttacker->ignoreBuildingDefense() : ignoreBuildingDefense(), false, defenseBuildingModifier());
/************************************************************************************************/
/* BETTER_BTS_AI_MOD                       END                                                  */
/************************************************************************************************/
			iModifier += iExtraModifier;
	...
	if (pAttacker != NULL && pAttackedPlot != NULL)
	{
		...
		{	
			// Civ4 Reimagined START
			CvCity* pAttackedCity = pAttackedPlot->getPlotCity();
			int iCityDefense;
			
			if (NO_IMPROVEMENT != pAttackedPlot->getImprovementType())
			{
				// Forts
				if (GC.getImprovementInfo(pAttackedPlot->getImprovementType()).isActsAsCity())
				{
					iExtraModifier = - std::min(pAttacker->cityAttackModifier(), (GC.getImprovementInfo(pAttackedPlot->getImprovementType())).getDefenseModifier());
				}
			}
			else
			{
				...
				iExtraModifier = - std::min(pAttacker->cityAttackModifier(), iCityDefense);
			}
				//iExtraModifier = - std::min(pAttacker->cityAttackModifier(), pAttackedCity->getDefenseModifier(pAttacker->ignoreBuildingDefense()));
			// Civ4 Reimagined END
		...
		if (!bAttackingUnknownDefender)
		{
			...
			if (pAttacker->collateralDamage() > 0 && ((GC.getUnitInfo(pAttacker->getUnitType())).getFirstStrikes() > GC.getUnitInfo(getUnitType()).getFirstStrikes()))
			{
				iTempModifier -= 500;
			}
...
// Civ4 Reimagined
int CvUnit::getCombatXPModifier() const
{
...
{
	int iMaxValue;
	bool bUnlimitedAnimalXP = false;
	bool bUnlimitedBarbXP = false;
	CvPlayer& kPlayer = GET_PLAYER(eWinner);
	
	if (eWinner != NO_PLAYER)
	{
		bUnlimitedAnimalXP = kPlayer.isUnlimitedAnimalXP();
		bUnlimitedBarbXP = kPlayer.isUnlimitedBarbXP();
	}

	iMaxValue = MAX_INT;

	if (isAnimal() && !bUnlimitedAnimalXP) // Civ4 Reimagined: Added unlimited experience from animals
	{
		iMaxValue = std::min(iMaxValue, GC.getDefineINT("ANIMAL_MAX_XP_VALUE"));
	}

	if (isBarbarian() && !bUnlimitedBarbXP) // Civ4 Reimagined: Added unlimited experience from barbarians
	{
...
void CvUnit::setXY(int iX, int iY, bool bGroup, bool bUpdate, bool bShow, bool bCheckPlotVisible)
{
	...
	if (pNewPlot != NULL)
	{
		...
		/* Civ4 Reimagined (moved to bottom, left here for convenience)
...
/************************************************************************************************/
	...
	if (pNewPlot != NULL)
	{
		...
		// Civ4 Reimagined: Copied part from above.
		if (pNewPlot->isGoody(getTeam()))
		{
			GET_PLAYER(getOwnerINLINE()).doGoody(pNewPlot, this);
		}
...
void CvUnit::changeExperience(int iChange, int iMax, bool bFromCombat, bool bInBorders, bool bUpdateGlobal)
{
	...
	if (bFromCombat)
	{
		...
		// Civ4 Reimagined
		if (getCombatXPModifier() != 0)
		{
			iUnitExperience *= std::max(0, 100 + getCombatXPModifier());
			iUnitExperience /= 100;
		}
	}
...
void CvUnit::setLevel(int iNewValue)
{
	if (getLevel() != iNewValue)
	{
		...
		//Civ4 Reimagined: Exclude scouts
		if(getUnitCombatType() != (UnitCombatTypes)GC.getInfoTypeForString("UNITCOMBAT_RECON"))
		{
		...
		//Civ4 Reimagined
		if (getDomainType() == DOMAIN_SEA && getLevel() > GET_PLAYER(getOwnerINLINE()).getHighestNavalUnitLevel())
		{
...
{
	return (getBlitzCount() + (m_pUnitInfo->isBlitz() ? 1 : 0) > 0);
}
...
void CvUnit::setHasPromotion(PromotionTypes eIndex, bool bNewValue)
{
	...
	if (isHasPromotion(eIndex) != bNewValue)
	{
		...
		changeNoUpgradeCount((GC.getPromotionInfo(eIndex).isNoUpgrade()) ? iChange : 0); // Leoreth
		...
		// Civ4 Reimagined
		if (! GET_TEAM(getTeam()).isTechBoosted((TechTypes)GC.getInfoTypeForString("TECH_OPTICS")))
		{
			if (bNewValue)
			{
				if (eIndex == (PromotionTypes)GC.getInfoTypeForString("PROMOTION_NAVIGATION1") || eIndex == (PromotionTypes)GC.getInfoTypeForString("PROMOTION_NAVIGATION2"))
				{
					GET_TEAM(getTeam()).setTechBoosted((TechTypes)GC.getInfoTypeForString("TECH_OPTICS"), getOwnerINLINE(), true);
				}
			}
		}
...
void CvUnit::collateralCombat(const CvPlot* pPlot, CvUnit* pSkipUnit)
{
	...
	if (iDamageCount > 0)
	{
		...
		gDLL->getInterfaceIFace()->addHumanMessage(pSkipUnit->getOwnerINLINE(), (pSkipUnit->getDomainType() != DOMAIN_AIR), GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_COLLATERAL", MESSAGE_TYPE_INFO, getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_YELLOW"), pSkipUnit->getX_INLINE(), pSkipUnit->getY_INLINE(), true, true);
...
bool CvUnit::canCoastalRaidAt(const CvPlot* pPlot, int iX, int iY) const
{
	...
	if (pTargetPlot->isOwned())
	{
		...
		if (GET_PLAYER(pTargetPlot->getOwnerINLINE()).isNoPillage())
		{
			return false;
		}
	}
...
bool CvUnit::coastalRaid(int iX, int iY)
{
	...
	if (gUnitLogLevel > 2)
		logBBAI("%S did coastal raid on plot (%d, %d)", getNameKey(), pPlot->getX_INLINE(), pPlot->getY_INLINE());
