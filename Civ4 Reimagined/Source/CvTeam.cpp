...
CvTeam::CvTeam()
{
	...
	m_abDefensiveWar = new bool[MAX_TEAMS]; // Civ4 Reimagined
...
CvTeam::~CvTeam()
{
	...
	SAFE_DELETE_ARRAY(m_abDefensiveWar); // Civ4 Reimagined
...
void CvTeam::reset(TeamTypes eID, bool bConstructorCall)
{
	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		...
		m_abAtWar[iI] = false;
		m_abDefensiveWar[iI] = false; // Civ4 Reimagined
		...
		if (!bConstructorCall && getID() != NO_TEAM)
		{
			...
			kLoopTeam.m_abDefensiveWar[getID()] = false; // Civ4 Reimagined
				...
				for (pLoopCity = GET_PLAYER((PlayerTypes)iI).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER((PlayerTypes)iI).nextCity(&iLoop))
				{
					for (iJ = 0; iJ < GC.getNumBuildingInfos(); iJ++)
					{
						if (pLoopCity->getNumBuilding((BuildingTypes)iJ) > 0)
						{
							...
							if (!isObsoleteBuilding((BuildingTypes)iJ) && !GET_PLAYER((PlayerTypes)iI).isWrongCivicBuilding((BuildingTypes)iJ))
							{
...
void CvTeam::processBuilding(BuildingTypes eBuilding, int iChange)
{
	...
	// Civ4 Reimagined
	if (GC.getBuildingInfo(eBuilding).isCanFarmHills())
	{
		changeCanFarmHillsCount(iChange);
		if (gTeamLogLevel > 2) logBBAI("CanFarmHills: %d", (int)isCanFarmHills());
	}
...
void CvTeam::declareWar(TeamTypes eTeam, bool bNewDiplo, WarPlanTypes eWarPlan, bool bPrimaryDoW)
{
	...
	// Civ4 Reimagined
	setDefensiveWar(getID(), false);
	GET_TEAM(eTeam).setDefensiveWar(getID(), true);
	...
	for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		if (GC.getMapINLINE().plotByIndexINLINE(iI)->getTeam() == getID())
		{
			GC.getMapINLINE().plotByIndexINLINE(iI)->forceBumpEnemyNavalUnits(eTeam);
		}

		if (GC.getMapINLINE().plotByIndexINLINE(iI)->getTeam() == eTeam)
		{
			GC.getMapINLINE().plotByIndexINLINE(iI)->forceBumpEnemyNavalUnits(getID());
		}
	}
		...
		for (PlayerTypes j = (PlayerTypes)0; j < MAX_PLAYERS; j = (PlayerTypes)(j+1))
		{
			...
			else if (kPlayer_j.getTeam() != getID())
			{
				...
				if (kTeam_j.isHasMet(eTeam) && !kTeam_j.isAtWar(eTeam))
				{
					...
					// Civ4 Reimagined: For reference: eTeam = attacked team. kPlayer_i: Player of attacking team. kPlayer_j: Not involved player who witnesses the attack.
					if (kTeam_j.AI_getAttitude(eTeam) >= ATTITUDE_ANNOYED && !(kPlayer_i.isHuman() && GET_TEAM(eTeam).isHuman()))
					{
						if ((kPlayer_i.isHuman() && GET_TEAM(eTeam).AI_getAttitude(kPlayer_i.getTeam()) >= ATTITUDE_FRIENDLY) ||
							(!kPlayer_i.isHuman() && GET_TEAM(kPlayer_i.getTeam()).AI_getAttitude(eTeam) >= ATTITUDE_FRIENDLY))
						{
							kPlayer_j.AI_changeMemoryCount(i, MEMORY_BACKSTABBED_ALLY, 10);
						}							
					}
				}
			}
...
void CvTeam::makePeace(TeamTypes eTeam, bool bBumpUnits)
{
	...
	if (isAtWar(eTeam))
	{
		...
		// Civ4 Reimagined
		setDefensiveWar(eTeam, false);
		GET_TEAM(eTeam).setDefensiveWar(getID(), false);
...
int CvTeam::getDefensivePower(TeamTypes eExcludeTeam, bool bConquest) const
{
	int iCount;
	int iI;
	...
	for (iI = 0; iI < MAX_CIV_TEAMS; iI++)
	{
		...
		if (kLoopTeam.isAlive()) // K-Mod. (vassal check unnecessary, b/c vassals can't be the master team, and they can't have a pact.)
		{
			...
			if (iI != eExcludeTeam && (kMasterTeam.getID() == iI || kMasterTeam.isDefensivePact((TeamTypes)iI))) // K-Mod
			{
				...
				// Civ4 Reimagined
				// Don't start wars easily against Nuclear Powers. Especially when their arsenal is big enough to destroy you.
				// Civ4 Reimagined To-Do: Maybe make this code more sophisticated later (SDI etc)
				if ( kLoopTeam.hasNukes() && !bConquest && eExcludeTeam != NO_TEAM)
				{
					const CvTeam& kExcludeMasterTeam = GET_TEAM(GET_TEAM((TeamTypes)eExcludeTeam).getMasterTeam());
					if ( kLoopTeam.getNumNukeUnits() > (kExcludeMasterTeam.getNumCities() / 2) )
					{
						iCount += kLoopTeam.getPower(false) * 5;
					}
				}
			}
		}
...
int CvTeam::getWarWeariness(TeamTypes eIndex, bool bUseEnemyModifer) const
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < MAX_TEAMS, "eIndex is expected to be within maximum bounds (invalid Index)");
	int iWarWeariness = m_aiWarWeariness[eIndex];
	if (bUseEnemyModifer)
	{
		iWarWeariness *= std::max(0, 100 + GET_TEAM(eIndex).getEnemyWarWearinessModifier())/100;
	}
	
	// Civ4 Reimagined
	/*
	if (isDefensiveWar(eIndex))
	{
		iWarWeariness *= std::max(0, 100 + getDefensiveWarWearinessModifier())/100;
	}
	*/
	
	return iWarWeariness; // K-Mod
}
...
// Civ4 Reimagined
bool CvTeam::isDefensiveWar(TeamTypes eIndex) const
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < MAX_TEAMS, "eIndex is expected to be within maximum bounds (invalid Index)");
	return m_abDefensiveWar[eIndex];
}

// Civ4 Reimagined
void CvTeam::setDefensiveWar(TeamTypes eIndex, bool bNewValue)
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < MAX_TEAMS, "eIndex is expected to be within maximum bounds (invalid Index)");
	m_abDefensiveWar[eIndex] = bNewValue;
}

/*
// Civ4 Reimagined
int CvTeam::getDefensiveWarWeariness() const
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < MAX_TEAMS, "eIndex is expected to be within maximum bounds (invalid Index)");
	return 100;
...
// Civ4 Reimagined
void CvTeam::changeDefensiveWarWeariness(int iChange)
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < MAX_TEAMS, "eIndex is expected to be within maximum bounds (invalid Index)");
}
		...
		for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
		{
			...
			if (kLoopPlayer.isAlive() && kLoopPlayer.getTeam() == eIndex)
			{
				...
				for (CvUnit* pLoopUnit = kLoopPlayer.firstUnit(&iLoop); NULL != pLoopUnit; pLoopUnit = kLoopPlayer.nextUnit(&iLoop))
				{
					...
					{
						if (!pLoopUnit->isNoUpkeep()) // Civ4 Reimagined
						{
			...
			for (iI = 0; iI < MAX_PLAYERS; iI++)
			{
				if (GET_PLAYER((PlayerTypes)iI).isAlive())
				{
					if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
					{
						...
						{
							// Civ4 Reimagined: Added wrong-civic case
							if (pLoopCity->getNumBuilding(eIndex) > 0 && !GET_PLAYER((PlayerTypes)iI).isWrongCivicBuilding(eIndex))
							{
...
int CvTeam::getNoConquestResistanceCount() const
{
	return m_iNoConquestResistanceCount;
}


bool CvTeam::isNoConquestResistance() const
{
	return (getNoConquestResistanceCount() > 0);
}

void CvTeam::changeNoConquestResistanceCount(int iChange)
{
	if (iChange != 0)
	{
		m_iNoConquestResistanceCount += iChange;
		FAssert(getNoConquestResistanceCount() >= 0);
	}
}
...
// Civ4 Reimagined
bool CvTeam::isCanFarmHills() const
{
	return (getCanFarmHillsCount() > 0);
}

// Civ4 Reimagined
void CvTeam::changeCanFarmHillsCount(int iChange)
{
	if (iChange != 0)
	{
		m_iCanFarmHillsCount += iChange;
		FAssert(getCanFarmHillsCount() >= 0);
	}
}
...
void CvTeam::setHasTech(TechTypes eIndex, bool bNewValue, PlayerTypes ePlayer, bool bFirst, bool bAnnounce)
{
	...
	if (isHasTech(eIndex) != bNewValue)
	{
		...
		{
			GET_PLAYER(ePlayer).updateExtraYieldThreshold((YieldTypes)iI);
		}
...
/************************************************************************************************/
			...
			if (bFirst)
			{
				if (GC.getGameINLINE().countKnownTechNumTeams(eIndex) == 1)
				{
					...
					if( gTeamLogLevel >= 2 )
					{
						logBBAI("    Team %d (%S) is the first to discover tech %S in %d %s", getID(), getName().GetCString(), GC.getTechInfo(eIndex).getDescription(), std::abs(GC.getGameINLINE().getGameTurnYear()), GC.getGameINLINE().getGameTurnYear()>0 ? "AD" : "BC");
					}
					CyArgsList argsList;
					argsList.add(getID());
...
void CvTeam::setTechBoosted(TechTypes eIndex, PlayerTypes ePlayer, bool bNewValue)
{
	...
	if (GC.getDefineINT("EUREKA_TECH_BOOST_PERCENTAGE") > 0)
	{
		if (bNewValue && bNewValue != m_pabTechBoosted[eIndex] && !isHasTech(eIndex))
		{
			...
			if (GC.getTechInfo(eIndex).getEra() == ERA_ANCIENT)
			{
...
// Civ4 Reimagined
int CvTeam::getBuildingYieldChange(BuildingClassTypes eIndex1, YieldTypes eIndex2) const
{
	FAssertMsg(eIndex1 >= 0, "eIndex1 is expected to be non-negative (invalid Index)");
...
// Civ4 Reimagined
void CvTeam::changeBuildingYieldChange(BuildingClassTypes eIndex1, YieldTypes eIndex2, int iChange)
{
	FAssertMsg(eIndex1 >= 0, "eIndex1 is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex1 < GC.getNumBuildingClassInfos(), "eIndex1 is expected to be within maximum bounds (invalid Index)");
	FAssertMsg(eIndex2 >= 0, "eIndex2 is expected to be non-negative (invalid Index)");
...
void CvTeam::processTech(TechTypes eTech, int iChange)
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
		{
			...
			GET_PLAYER((PlayerTypes)iI).changeSeaPlotYield(YIELD_COMMERCE, iChange * GC.getTechInfo(eTech).getCoastBonusCommerce()); // Civ4 Reimagined
			...
			if (!GET_PLAYER((PlayerTypes)iI).canExploreSea())
			{
				...
				{
					eLoopUnit = (UnitTypes)GC.getCivilizationInfo(GET_PLAYER((PlayerTypes)iI).getCivilizationType()).getCivilizationUnits(iJ);
					if (eLoopUnit != NO_UNIT)
					{
						if (GC.getUnitInfo(eLoopUnit).getDomainType() == DOMAIN_SEA && GC.getUnitInfo(eLoopUnit).getPrereqAndTech() == eTech)
						{
							if(!GC.getUnitInfo(eLoopUnit).getTerrainImpassable(TERRAIN_OCEAN))
							{
								GET_PLAYER((PlayerTypes)iI).setCanExploreSea(true);
							}
						}
					}
				}
...
void CvTeam::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(MAX_TEAMS, m_abDefensiveWar); // Civ4 Reimagined
...
void CvTeam::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(MAX_TEAMS, m_abDefensiveWar); // Civ4 Reimagined
...
// Civ4 Reimagined
bool CvTeam::hasNukes() const
{
	BuildingTypes eBuilding = (BuildingTypes)GC.getInfoTypeForString("BUILDING_NUCLEAR_RESEARCH_CENTER");
	return (getBuildingClassCount((BuildingClassTypes)GC.getBuildingInfo(eBuilding).getBuildingClassType()) > 0);
}

// Civ4 Reimagined
void CvTeam::announceFirstDiscoveredTech(TechTypes eTech, PlayerTypes ePlayer) const
{
	for (int iI = 0; iI < MAX_PLAYERS; ++iI)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			...
			{
				szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_FIRST_TO_TECH", GET_PLAYER(ePlayer).getNameKey(), GC.getTechInfo(eTech).getTextKeyWide());
			}
			else
			{
