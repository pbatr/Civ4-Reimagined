...
void CyCityPythonInterface1(python::class_<CyCity>& x)
{
	...
	x
		...
		.def("foodDifference", &CyCity::foodDifference, "int (bool bBottom, bool bIgnoreFoodProduction) - result of getYieldRate(Food) - foodConsumption()")
		...
		.def("getTradeCultureRateTimes100", &CyCity::getTradeCultureRateTimes100, "int (iLevel) - returns the trade culture output, given the culture level of the destination city") /* K-Mod */
		...
		.def("calculateTradeProfitTimes100", &CyCity::calculateTradeProfitTimes100, "int (CyCity) - returns the trade profit times 100 created by CyCity") // Civ4 Reimagined
