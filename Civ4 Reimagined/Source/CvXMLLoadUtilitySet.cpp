...
bool CvXMLLoadUtility::LoadPreMenuGlobals()
{
	...
	LoadGlobalClassInfo(GC.getIdeologyInfo(), "CIV4IdeologyInfo", "GameInfo", "Civ4IdeologyInfo/IdeologyInfos/IdeologyInfo", false);
	...
	LoadGlobalClassInfo(GC.getBuildingInfo(), "CIV4BuildingInfos", "Buildings", "Civ4BuildingInfos/BuildingInfos/BuildingInfo", false, NULL, true);
...
// < Building Civic Prereqs Start >
template <class T>
void CvXMLLoadUtility::UpdateGlobalClassInfo(std::vector<T*>& aInfos, const char* szTagName)
{
	char szLog[256];
	sprintf(szLog, "UpdateGlobalClassInfo (%s)", szTagName);
	PROFILE(szLog);
	logMsg(szLog);
...
template <class T>
// < Building Civic Prereqs Start >
void CvXMLLoadUtility::LoadGlobalClassInfo(std::vector<T*>& aInfos, const char* szFileRoot, const char* szFileDirectory, const char* szXmlPath, bool bTwoPass, CvCacheObject* (CvDLLUtilityIFaceBase::*pArgFunction) (const TCHAR*), bool bFourPass)
// < Building Civic Prereqs End   >
{
	// < Building Civic Prereqs Start >
	if(bFourPass)
	{
		LoadCivXml(m_pFXml, CvString::format("xml\\%s/%s.xml", szFileDirectory, szFileRoot));
		UpdateGlobalClassInfo(aInfos, szXmlPath);
		return;
	}
