...
void CvDLLWidgetData::parseHelp(CvWStringBuffer &szBuffer, CvWidgetDataStruct &widgetDataStruct)
{
	...
	// Civ4 Reimagined
	case WIDGET_HELP_SEA_PLOT_YIELD:
		parseSeaPlotYieldHelp(widgetDataStruct, szBuffer);
		break;
	
	// Civ4 Reimagined
	case WIDGET_HELP_ENABLE_IDEOLOGIES:
		parseIdeologiesHelp(widgetDataStruct, szBuffer);
		break;
...
//		GC.getDiplomacyScreen().handleClick(m_pWidget);
	...
	case WIDGET_HELP_IGNORE_IRRIGATION:
	case WIDGET_HELP_WATER_WORK:
	case WIDGET_HELP_SEA_PLOT_YIELD: // Civ4 Reimagined
	case WIDGET_HELP_IMPROVEMENT:
	case WIDGET_HELP_DOMAIN_EXTRA_MOVES:
	case WIDGET_HELP_ADJUST:
	case WIDGET_HELP_TERRAIN_TRADE:
			...
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() == MISSION_DISCOVER)
			{
				...
				while (pSelectedUnitNode != NULL)
				{
					...
					if (pSelectedUnit->canDiscover(pMissionPlot))
					{
						...
						szBuffer.append(NEWLINE);
						szBuffer.append(gDLL->getText("TXT_KEY_ACTION_EXTRA_RESEARCH", pSelectedUnit->getDiscoverResearch(NO_TECH), GC.getTechInfo(eTech).getTextKeyWide()));
				...
				for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
				{
					...
					if (NO_FEATURE != pMissionPlot->getFeatureType())
					{
						if (GC.getBuildInfo(eBuild).isFeatureRemove(pMissionPlot->getFeatureType()))
						{
							...
							{
								iYield -= GET_PLAYER(pMissionPlot->getOwnerINLINE()).getFeatureExtraYield(pMissionPlot->getFeatureType(), (YieldTypes)iI);
							}
						}
					}
...
/*************************************************************************************************/
				...
				if (eImprovement != NO_IMPROVEMENT)
				{
					...
					else
					{
						...
						{
							if (GET_TEAM(pHeadSelectedUnit->getTeam()).isBonusRevealed((BonusTypes) iI))
							{
		...
		if (widgetDataStruct.m_iData2 > 0)
		{
			...
			if (widgetDataStruct.m_iData1 != GC.getDefineINT("DEFAULT_SPECIALIST"))
			{
				...
				//if (!(GET_PLAYER(pHeadSelectedCity->getOwnerINLINE()).isSpecialistValid((SpecialistTypes)(widgetDataStruct.m_iData1))))
					if (pHeadSelectedCity->getMaxSpecialistCount((SpecialistTypes)(widgetDataStruct.m_iData1)) < MAX_INT && 
						pHeadSelectedCity->getMaxSpecialistCount((SpecialistTypes)(widgetDataStruct.m_iData1)) > 0)
					{
						szBuffer.append(NEWLINE);
...
void CvDLLWidgetData::parseScoreboardCheatText(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	...
	if (kPlayer.AI_isDoStrategy(AI_STRATEGY_SPECIALIST_ECONOMY)) // Civ4 Reimagined
	{
...
/************************************************************************************************/
		...
		case TRADE_PEACE_TREATY:
			szBuffer.append(gDLL->getText("TXT_KEY_TRADE_PEACE_TREATY", GC.getGameINLINE().getPeaceDealLength())); // Civ4 Reimagined: Added gamespeed modifier
			break;
		}
...
// Civ4 Reimagined
void CvDLLWidgetData::parseBonusRatioHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity = gDLL->getInterfaceIFace()->getHeadSelectedCity();
	if (NULL != pHeadSelectedCity)
	{
		int iTechValue = GET_PLAYER(pHeadSelectedCity->getOwnerINLINE()).getTechValue();
		int iTotalPopulation = GET_PLAYER(pHeadSelectedCity->getOwnerINLINE()).getTotalPopulation();
		szBuffer.assign(gDLL->getText("TXT_KEY_MISC_BONUS_RATIO_INFO"));
		szBuffer.append(NEWLINE);
		szBuffer.append(NEWLINE);
		CvWString szTechValueString = CvWString::format(L"%d.%02d", iTechValue/100, iTechValue%100);
		szBuffer.append(gDLL->getText("TXT_KEY_MISC_BONUS_RATIO_TECH_VALUE",  szTechValueString.GetCString()));
		szBuffer.append(NEWLINE);
		...
		{
			szBuffer.append(L"\n-----------------------\n");
			CvWString szBaseRatioString = CvWString::format(L"%d.%02d", iBaseRatio/100, iBaseRatio%100);
			szBuffer.append(gDLL->getText("TXT_KEY_MISC_BONUS_RATIO_BASE_RATIO", szBaseRatioString.GetCString()));
			szBuffer.append(NEWLINE);
			if (iBonusValueModifier > 0)
			{
				szBuffer.append(gDLL->getText("TXT_KEY_MISC_BONUS_RATIO_MULTIPLIER_POSITIVE", iBonusValueModifier));
			}
			else
			{
		...
		if (iTotalRatio > 100)
		{
			szBuffer.append(gDLL->getText("TXT_KEY_MISC_BONUS_RATIO_CAPPED"));
		}
...
// Civ4 Reimagined
void CvDLLWidgetData::parseSeaPlotYieldHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	GAMETEXT.buildSeaPlotYieldString(szBuffer, ((TechTypes)(widgetDataStruct.m_iData1)));
}

// Civ4 Reimagined
void CvDLLWidgetData::parseIdeologiesHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	GAMETEXT.buildIdeologiesString(szBuffer, ((TechTypes)(widgetDataStruct.m_iData1)));
}
