...
void CvPlayerAI::AI_updateAttitudeCache(PlayerTypes ePlayer)
{
	...
	if (!(kPlayer.isHuman()))
	{
		iAttitude += (3 - abs(AI_getPeaceWeight() - kPlayer.AI_getPeaceWeight()));
...
/************************************************************************************************/
	...
	case UNITAI_RESERVE_SEA:
		...
		if (GC.getUnitInfo(eUnit).getInvisibleType() != NO_INVISIBLE || GC.getUnitInfo(eUnit).isInvisible())
		{
			iValue += 100;
		}
...
/************************************************************************************************/
	...
	{
		// Civ4 Reimagined
		int iMissileCarrierValue = (GC.getUnitInfo(eUnit).isInvisible()) ? 300 : iCombatValue;
	...
	for (HurryTypes i = (HurryTypes)0; i < GC.getNumHurryInfos(); i=(HurryTypes)(i+1))
	{
		if (kCivic.isHurry(i))
		{
			...
			if (kHurryInfo.getGoldPerProduction() > 0)
			{
				...
				int iHurryModifiers = getHurryModifier() + kHurryInfo.isUnits() ? getMercenaryCostModifier() : getBuyBuildingCostModifier();
...
int CvPlayerAI::AI_countPotentialForeignTradeCities(bool bCheckConnected, bool bCheckForeignTradePolicy, CvArea* pIgnoreArea) const
{
	...
	for (PlayerTypes i = (PlayerTypes)0; i < MAX_CIV_PLAYERS; i = (PlayerTypes)(i+1))
	{
		...
		if (bCheckForeignTradePolicy && (kLoopPlayer.isNoForeignTrade() || isIgnoreForeignTradeBan()) && !kTeam.isVassal(kLoopPlayer.getTeam()) && !GET_TEAM(kLoopPlayer.getTeam()).isVassal(getTeam()))
			continue;
