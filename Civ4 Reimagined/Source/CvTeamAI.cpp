...
int CvTeamAI::AI_chooseElection(const VoteSelectionData& kVoteSelectionData) const
{
	...
	for (int iI = 0; iI < (int)kVoteSelectionData.aVoteOptions.size(); ++iI)
	{
		...
		if (bValid)
		{
			// Civ4 Reimagined To-Do: some kind of weighting mechanism, don't choose completely random
...
int CvTeamAI::AI_warSpoilsValue(TeamTypes eTarget, WarPlanTypes eWarPlan) const
{
	...
	int iDenyFactor = bAggressive
		? 40 - AI_getAttitudeWeight(eTarget)/3
		: 20 - AI_getAttitudeWeight(eTarget)/2;

	iDenyFactor += AI_getWorstEnemy() == eTarget ? 20 : 0; // (in addition to attitude penalities)
	...
	if (kTargetTeam.AI_isAnyMemberDoVictoryStrategyLevel3())
	{
		if (kTargetTeam.AI_isAnyMemberDoVictoryStrategyLevel4())
		{
			...
			iDenyFactor += AI_isAnyMemberDoVictoryStrategyLevel4() ? 75 : 45;
		}

		// Civ4 Reimagined
		if (AI_isAnyMemberDoVictoryStrategy(AI_VICTORY_CONQUEST3 | AI_VICTORY_DOMINATION3))
		{
			iDenyFactor += bAggressive ? 40 : 20;
		}
	...
	iGainedValue /= 1 + kTargetTeam.getAtWarCount(true, true); //Civ4 Reimagined
	iDeniedValue /= 1 + kTargetTeam.getAtWarCount(true, true); //Civ4 Reimagined
	
	// Civ4 Reimagined: also decrease the gain for distant targets...
	if (AI_teamCloseness(eTarget) == 0)
	{
		...
		for (PlayerTypes eLoopPlayer = (PlayerTypes)0; eLoopPlayer < MAX_PLAYERS; eLoopPlayer=(PlayerTypes)(eLoopPlayer+1))
		{
			...
			if (kLoopPlayer.getTeam() == getID() && kLoopPlayer.isAlive())
			{
				bestEra = std::max(bestEra,(int)kLoopPlayer.getCurrentEra());
			}
		}
...
int CvTeamAI::AI_warCommitmentCost(TeamTypes eTarget, WarPlanTypes eWarPlan) const
{
	...
	// Estimate of military production costs
	{
		...
		// cf. with code in AI_calculateAreaAIType
		{
			...
			for (TeamTypes i = (TeamTypes)0; i < MAX_CIV_TEAMS; i=(TeamTypes)(i+1))
			{
				...
				if (isAtWar(i) || i == eTargetMaster || kLoopTeam.isVassal(eTargetMaster) || (bPendingDoW && kLoopTeam.isDefensivePact(eTargetMaster)))
				{
					...
					if ( kLoopTeam.hasNukes() )
					{
						if ( (kLoopTeam.getNumNukeUnits() > (getNumCities() / 2)) && !(AI_isAnyMemberDoVictoryStrategy(AI_VICTORY_DOMINATION4 | AI_VICTORY_CONQUEST4)) )
						{
							iEnemyRelativeStrength += kLoopTeam.getPower(false) * 5;
						}
						else
						{
							iEnemyRelativeStrength += kLoopTeam.getPower(false);
						}
					}
				}
			}
		...
		if (AI_teamCloseness(eTarget) == 0)
		{
			...
			// Civ4 Reimagined. Multiply with *4 (ancient) to *2 (future)
			iCommitmentPerMil = iCommitmentPerMil * (((8-bestEra)/2)+5) / 2;
	...
	if (iTotalWw > 50)
	{
		...
		for (PlayerTypes eLoopPlayer = (PlayerTypes)0; eLoopPlayer < MAX_CIV_PLAYERS; eLoopPlayer=(PlayerTypes)(eLoopPlayer+1))
		{
			...
			if (kLoopPlayer.getTeam() == getID() && kLoopPlayer.isAlive())
			{
				...
				iWwCost += iS * kLoopPlayer.getNumCities() * kLoopPlayer.AI_getHappinessWeight(iS * iEstimatedPercentAnger * (100 + kLoopPlayer.getWarWearinessModifier())/100, 0, 0, true) / 20;
			}
		}
		iTotalCost += iWwCost;
	}
...
int CvTeamAI::AI_endWarVal(TeamTypes eTeam) const
{
	...
	//Civ4 Reimagined
	int iTheirPower = std::max(1, kWarTeam.getPower(true));
	...
	//Civ4 Reimagined
	iValue *= ( kWarTeam.hasNukes() && !hasNukes() ) ? 2 : 1;
	iValue *= ( (kWarTeam.getNumNukeUnits() > 1) && !hasNukes() ) ? 2 : 1;
	...
	for (TeamTypes i = (TeamTypes)0; i < MAX_CIV_TEAMS; i=(TeamTypes)(i+1))
	{
		...
		else if (!kMasterTeam.isAtWar(i) && isAtWar(i))
		{
			...
			if (!kLoopTeam.isAVassal())
			{
				...
				// Civ4 Reimagined
				// Only peace vassal if we plan a war against the attacker.
				if (kMasterTeam.AI_getWarPlan(i) == NO_WARPLAN)
					return DENIAL_NO_GAIN;
...
/************************************************************************************************/
		...
		if (isAtWar(eTeam))
		{
			...
			if (iOurSuccess * 3 > iTheirSuccess * 2)
			{
				iMasterPower *= 2;
				iMasterPower /= 3;
			}
...
int CvTeamAI::AI_getWarSuccessRating() const
{
	...
	for (int iI = 0; iI < MAX_CIV_TEAMS; iI++)
	{
		...
		if (iI != getID() && isAtWar((TeamTypes)iI) && kLoopTeam.isAlive() && !kLoopTeam.isAVassal())
		{
			...
			// Civ4 Reimagined
			if ( kLoopTeam.hasNukes() && !hasNukes() )
			{
				if (iScore > 0)
					iScore /= 3;
				else
		...
		if( iI != getID() )
		{
			if( GET_TEAM((TeamTypes)iI).isAlive() && isHasMet((TeamTypes)iI) )
			{
				...
				else if( AI_isChosenWar((TeamTypes)iI) )
				{
					...
					// Civ4 Reimagined
					int iTempPower = 240 * GET_TEAM((TeamTypes)iI).getDefensivePower(getID(), AI_isAnyMemberDoVictoryStrategy(AI_VICTORY_CONQUEST3 | AI_VICTORY_CONQUEST4));
					iTempPower /= (AI_hasCitiesInPrimaryArea((TeamTypes)iI) ? 2 : 3);
...
/************************************************************************************************/
	...
	if (bConsiderPower)
	{
		...
		// Civ4 Reimagined
		if ((GET_TEAM(eWarTeam).getDefensivePower(getID(), AI_isAnyMemberDoVictoryStrategy(AI_VICTORY_CONQUEST3 | AI_VICTORY_CONQUEST4)) / ((bLandTarget) ? 2 : 1)) >
	...
	for (TeamTypes eLoopTeam = (TeamTypes)0; eLoopTeam < MAX_CIV_TEAMS; eLoopTeam=(TeamTypes)(eLoopTeam+1))
	{
		...
		else if (AI_getWarPlan(eLoopTeam) == WARPLAN_LIMITED || AI_getWarPlan(eLoopTeam) == WARPLAN_DOGPILE)
		{
			if( !isAtWar(eLoopTeam) )
			{
				...
				{
					if( GET_TEAM(eLoopTeam).getAtWarCount(true) == 0 )
					{
						if( gTeamLogLevel >= 1 )
						{
				...
				for (int iPass = 0; iPass < 3; iPass++)
				{
					for (int iI = 0; iI < MAX_CIV_TEAMS; iI++)
					{
						if (canEventuallyDeclareWar((TeamTypes)iI) && AI_haveSeenCities((TeamTypes)iI))
						{
							...
							{
								// Civ4 Reimagined
								int iDefensivePower = (GET_TEAM((TeamTypes)iI).getDefensivePower(getID(), AI_isAnyMemberDoVictoryStrategy(AI_VICTORY_CONQUEST3 | AI_VICTORY_CONQUEST4)) * 2) / 3;
				...
				for (int iI = 0; iI < MAX_CIV_TEAMS; iI++)
				{
					if (canEventuallyDeclareWar((TeamTypes)iI) && AI_haveSeenCities((TeamTypes)iI))
					{
						...
						if (iNoWarRoll >= AI_noWarAttitudeProb(AI_getAttitude((TeamTypes)iI)) && (!bVassal || iNoWarRoll >= AI_noWarAttitudeProb(AI_getAttitude(eLoopMasterTeam))))
						{
							...
							{
								// Civ4 Reimagined
								if (GET_TEAM((TeamTypes)iI).getDefensivePower(getID(), AI_isAnyMemberDoVictoryStrategy(AI_VICTORY_CONQUEST3 | AI_VICTORY_CONQUEST4)) < ((iOurPower * AI_limitedWarPowerRatio()) / 100))
								{
						...
						if (iNoWarRoll >= AI_noWarAttitudeProb(AI_getAttitude((TeamTypes)iI)) && (!bVassal || iNoWarRoll >= AI_noWarAttitudeProb(AI_getAttitude(eLoopMasterTeam))))
						{
							if (GET_TEAM((TeamTypes)iI).getAtWarCount(true) > 0)
							{
								if (AI_isLandTarget((TeamTypes)iI) || GET_TEAM((TeamTypes)iI).AI_isAnyMemberDoVictoryStrategyLevel4())
								{
									...
									// Civ4 Reimagined
									FAssert(GET_TEAM((TeamTypes)iI).getPower(true) == GET_TEAM((TeamTypes)iI).getDefensivePower(getID(), AI_isAnyMemberDoVictoryStrategy(AI_VICTORY_CONQUEST3 | AI_VICTORY_CONQUEST4)) || GET_TEAM((TeamTypes)iI).isAVassal());

									// Civ4 Reimagined
									if (((GET_TEAM((TeamTypes)iI).getDefensivePower(getID(), AI_isAnyMemberDoVictoryStrategy(AI_VICTORY_CONQUEST3 | AI_VICTORY_CONQUEST4)) * 3) / 2) < iDogpilePower)
									{
