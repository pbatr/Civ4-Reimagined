...
private:
	...
	template <class T>
	void UpdateGlobalClassInfo(std::vector<T*>& aInfos, const char* szTagName);
	...
	template <class T>
	// < Building Civic Prereqs Start >
	void LoadGlobalClassInfo(std::vector<T*>& aInfos, const char* szFileRoot, const char* szFileDirectory, const char* szXmlPath, bool bTwoPass, CvCacheObject* (CvDLLUtilityIFaceBase::*pArgFunction) (const TCHAR*) = NULL, bool bFourPass = false);
	// < Building Civic Prereqs End   >
#endif
