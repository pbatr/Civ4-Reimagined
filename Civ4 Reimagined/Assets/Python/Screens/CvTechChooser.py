...
# BUG - Tech Screen Resolution - start
		if (BugOpt.isWideTechScreen() and screen.getXResolution() > 1024):
			...
			xPanelWidth = screen.getXResolution() - 30
		yPanelHeight = 768
...
# BUG - 3.19 No Espionage - end
		...
		if ( gc.getTechInfo(i).getCoastBonusCommerce() != 0):
			szSeaPlotYieldButton = self.getNextWidgetName("SeaPlotYield")
			screen.addDDSGFCAt( szSeaPlotYieldButton, szTechRecord, ArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_WATERWORK").getPath(), iX + fX, iY + Y_ROW, TEXTURE_SIZE, TEXTURE_SIZE, WidgetTypes.WIDGET_HELP_SEA_PLOT_YIELD, i, -1, False )
			fX += X_INCREMENT
		...
		for j in range(gc.getNumBuildInfos()):
			...
			# Civ4 Reimagined
			if (gc.getBuildInfo(j).getImprovement() == gc.getInfoTypeForString("IMPROVEMENT_TERRACE")):
				bTechFound = 0
