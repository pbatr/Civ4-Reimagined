...
class CvCorporationScreen:
	...
	def __init__(self):
		...
		self.BORDER_WIDTH = 2
		self.BUTTON_SIZE = 40
		self.HIGHLIGHT_EXTRA_SIZE = 8

		self.X_SCREEN = 500 #500
		self.Y_SCREEN = 396 #396
		self.W_SCREEN = 1144 #1024
		self.H_SCREEN = 768 #768
		self.Z_SCREEN = -6.1
		...
		self.X_CORPORATION_START = 62 # edead (155) # Leoreth (70) # NP (65)
		self.DX_CORPORATION = 100 # edead (116) # Leoreth (113)
		self.Y_CORPORATION = 35 # edead (35)
		self.Y_GREAT_PERSON = 100
		self.Y_BONUSES = 65 # edead (77)
		self.Y_FOUNDED = 122
		self.Y_HEADQUARTERS = 142
		self.Y_CORPORATION_NAME = 58

		self.X_CORPORATION_AREA = 20
		self.Y_CORPORATION_AREA = 84
		self.W_CORPORATION_AREA = 1119
	...
	def drawCorporationInfo(self):
		...
		for i in range(gc.getNumCorporationInfos()):
			...
			# Civ4 Reimagined
			if gc.getCorporationInfo(i).isSlaves():
				if iNum == 0:
					szList = u""
				else:
					szList += u", "
				iNum += 1
				szList += u"%c" % CyGame().getSymbolID(FontSymbols.SLAVE_CHAR)
		...
		# Founded...
		#screen.setLabelAt("", szArea, localText.getText("TXT_KEY_RELIGION_SCREEN_DATE_FOUNDED", ()), CvUtil.FONT_LEFT_JUSTIFY, self.LEFT_EDGE_TEXT, self.Y_FOUNDED, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1) #Civ4 Reimagined
		...
		# Headquarters
		#screen.setLabelAt("", szArea, localText.getText("TXT_KEY_CORPORATION_SCREEN_HEADQUARTERS", ()), CvUtil.FONT_LEFT_JUSTIFY, self.LEFT_EDGE_TEXT, self.Y_HEADQUARTERS, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1) #Civ4 Reimagined
