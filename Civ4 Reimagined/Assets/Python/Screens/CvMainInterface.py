						...
						for k in range(pHeadSelectedCity.getNumBuilding(i)):
							...
							#K-Mod end
								...
								for j in range( YieldTypes.NUM_YIELD_TYPES):
									iYield = gc.getBuildingInfo(i).getYieldChange(j) + pHeadSelectedCity.getNumBuilding(i) * (pHeadSelectedCity.getBuildingYieldChange(gc.getBuildingInfo(i).getBuildingClassType(), j) + gc.getPlayer(pHeadSelectedCity.getOwner()).getBuildingYieldChange(gc.getBuildingInfo(i).getBuildingClassType(), j))
...
# BUG - Raw Yields - end
				...
				for i in range( gc.getNumBonusInfos() ):
					...
					if ( pHeadSelectedCity.hasBonus(i) ):
						...
						#Civ4 Reimagined: Display individual bonus ratios
						if (iHappiness != 0):
							if ( iHappiness > 0 ):
								szTempBuffer = u"<font=1>+%d.%02d %c</font>" %(iHappiness/100, iHappiness%100, CyGame().getSymbolID(FontSymbols.HAPPY_CHAR) )
							else:
								szTempBuffer = u"<font=1>+%d.%02d %c</font>" %( -iHappiness/100, iHappiness%100, CyGame().getSymbolID(FontSymbols.UNHAPPY_CHAR) )

							if ( iHealth > 0 ):
								szTempBuffer += u"<font=1>+%d.%02d %c</font>" %( iHealth/100, iHealth%100, CyGame().getSymbolID( FontSymbols.HEALTHY_CHAR ) )
							...
							if ( iHealth > 0):
								screen.setLabelAt( szName, "BonusBack2", szTempBuffer, CvUtil.FONT_RIGHT_JUSTIFY, 133, (iRightCount * 20) + 4, -0.1, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, i, -1 )
							else:
								screen.setLabelAt( szName, "BonusBack2", szTempBuffer, CvUtil.FONT_RIGHT_JUSTIFY, 86, (iRightCount * 20) + 4, -0.1, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, i, -1 )
						...
						if (iHealth != 0 and bHandled == False):
							if ( iHealth > 0 ):
								...
								szTempBuffer = u"<font=1>+%d.%02d %c</font>" %( -iHealth/100, iHealth%100, CyGame().getSymbolID(FontSymbols.UNHEALTHY_CHAR) )
							...
							screen.setLabelAt( szName, "BonusBack1", szTempBuffer, CvUtil.FONT_RIGHT_JUSTIFY, 85, (iCenterCount * 20) + 4, -0.1, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, i, -1 )
