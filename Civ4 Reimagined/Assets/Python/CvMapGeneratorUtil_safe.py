	...
	def normalizeAddExtras(self):
		...
		for i in range(self.gc.getMAX_CIV_PLAYERS()):
			if (self.gc.getPlayer(i).isAlive()):
				...
				for dx in range(-4,4):
					for dy in range(-4,4):
						...
						if not pLoopPlot.isNone():
							...
							plotBonus = pLoopPlot.getBonusType(-1)
				...
				#	BonusType bonus = pLoopPlot.getBonusType(-1)
				#if(pLoopPlot.getBonusType(-1) is not None):
				#	for (pLoopPlot) in plots: #Civ4 Reimagined: Don't add a balancing resource if there is already one in the area.
				#		if(self.gc.getBonusInfo().getType() == type_string):
